// **************************************************************************
//
//  WRT54G.C - WRT54G/GS EJTAG Debrick Utility  v4.11
//
//  Note:
//  This program is for De-Bricking the WRT54G/GS and other misc routers.
//
//  New for 4.11    - Added buspirate interface
//                    - Added the following New Switch Options
//                      - /buspirate     enable buspirate
//                      - /bpdevice      set buspirate device default /dev/ttyUSB0
//
...
int bigendianfile    = 0;
int buspirate        = 0;
char asDevice[MAX_BP_DEVICE] = "/dev/ttyUSB0";
...
void lpt_openport(void)
{
    if(buspirate)
    {
        ...
        perror("BusPirate is unsupported on Windows");
        exit(1);
   #else                     // ---- Compiler Specific Code ----
        pfd = open(asDevice, O_RDWR | O_NOCTTY | O_NDELAY);
        if (pfd < 0)
        {
            fprintf(stderr,"Failed to open %s\n",asDevice);
            exit(1);
        }
        buspirate_serial_setspeed(pfd);
        if(!buspirate_binary_protocol(pfd))
            exit(1);
        buspirate_set_direction(pfd);
   #endif
    }
    else
    {
...
{
    if(buspirate)
        buspirate_binary_reset(pfd);
   #ifndef WINDOWS_VERSION   // ---- Compiler Specific Code ----

      #ifndef __FreeBSD__    // ---- Compiler Specific Code ----

         if (!buspirate && ioctl(pfd, PPRELEASE) < 0)  {  perror("Failed to release /dev/parport0");  close(pfd);  exit(0);  }
...
static unsigned char clockin(int tms, int tdi)
{
    ...
    if(buspirate)
    {
        ...
        */
        char buf[1] = { 0x80 }; // 1 XXXXXXX
        int ret = 0;

        //        MOSI       | CLK      | MISO     | CS
        buf[0] |= (tdi << 3) | (0 << 2) | (0 << 1) | (tms << 0); // clock low

        buspirate_serial_write(pfd,buf,1);
        //usleep(1);
        ret = buspirate_serial_read(pfd, buf, 1); // dummy read
        if (ret != 1) {
            fprintf(stderr,"error read serial on bus pirate\n");
        }
        buf[0] = 0x80;
        //        MOSI       | CLK      | MISO     | CS
        buf[0] |= (tdi << 3) | (1 << 2) | (0 << 1) | (tms << 0); // clock high return TDO

        buspirate_serial_write(pfd,buf,1);
        //usleep(1);
        ret = buspirate_serial_read(pfd, buf, 1);
        if (ret != 1) {
            fprintf(stderr,"error read serial on bus pirate\n");
        }
        data = (buf[0] & 2) >> 1;
...
// ---------------------------------------
// ---- BUS PIRATE Functions          ----
// ---------------------------------------

int buspirate_binary_reset(int fd)
{
    char tmp[5];
    ...
    usleep(10000);
    buspirate_serial_read(fd, tmp, 5); // dummy read
    if (strncmp(tmp, "BBIO1", 5) == 0)
    {
        ...
        fprintf(stderr,"Unable to restart buspirate!\n");
    return 0;
}

void buspirate_serial_setspeed(int fd)
{
...
int buspirate_binary_protocol(int fd)
{
    int ret;
    ...
    if (ret != 4) 
    {
        fprintf(stderr,"Buspirate error. Is binary support enabled?\n");
        return 0;
    }
    if (strncmp(tmp, "BBIO", 4) == 0) 
    {
        ...
        if (ret != 1) 
        {
            fprintf(stderr,"Buspirate did not answer correctly! Do you have correct firmware?\n");
            return 0;
        }
        if (tmp[0] != '1') 
        {
            fprintf(stderr,"Unsupported binary protocol, returned version %c\n",tmp[0]);
            return 0;
        }
    }
    return 1;
...
int buspirate_serial_write(int fd, char *buf, int size)
{
    ...
    if (ret != size)
        fprintf(stderr,"Error sending data on bus pirate\n");
...
int buspirate_serial_read(int fd, char *buf, int size)
{
    ...
    if (len != size)
        fprintf(stderr,"Error reading data on bus pirate\n");
...
void show_usage(void)
{
   ...
   printf( " USAGE: wrt54g [parameter] </noreset> </noemw> </nocwd> </nobreak> </noerase>\n"
	   ...
	   "            /bigendian ......... cpu is bigendian not littleendian\n"
           ...
           "            /buspirate.......... use bus pirate instead of parport. BusPirate actually doesn't works on Windows\n"
           "            /bpdevice:/dev/ttyXX set serial device of bus pirate default is /dev/ttyUSB0\n\n"
...
int main(int argc, char** argv)
{
    ...
    if (argc > 2)
    {
       ...
       while (j < argc)
       {
          ...
          else if (strcasecmp(choice,"/buspirate")==0)       buspirate = 1;
          else if (strncasecmp(choice,"/bpdevice:",10)==0)   strncpy(asDevice,choice + 10,MAX_BP_DEVICE);
          else
          {
             show_usage();
             printf("\n*** ERROR - Invalid <option> specified ***\n\n");
